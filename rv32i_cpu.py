...
class FetchStage(Module):
    ...
    def build(self, pipeline_regs, imem, decode_stage):
        ...
        # 不再使用stall信号，因为HazardUnit直接控制pipeline_reg的valid位
        # 直接获取指令，由HazardUnit控制if_id_valid
        ...
        # ID阶段 - 直接使用pipeline_regs[0].if_id_valid控制执行
        # HazardUnit会直接修改if_id_valid来控制流水线
        id_valid_in = pipeline_regs[0].if_id_valid
...
class DecodeStage(Module):
    ...
    def build(self, pipeline_regs, reg_file, execute_stage):
        ...
        # EX阶段 - 直接使用pipeline_regs[0].id_ex_valid控制执行
        # HazardUnit会直接修改id_ex_valid来控制流水线
        ex_valid_in = pipeline_regs[0].id_ex_valid
        ...
        needs_rs1 = (control_signals[4:0] != UInt(5)(0)) | (control_signals[19:17] != UInt(3)(0)) | (control_signals[20:20] == UInt(1)(1))  # ALU操作、分支操作或跳转操作需要rs1
        needs_rs2 = (control_signals[10:9] == UInt(2)(0)) & ((control_signals[4:0] != UInt(5)(0)) | (control_signals[19:17] != UInt(3)(0)))  # 只有当ALU源是寄存器且是ALU或分支操作时才需要rs2
        
        with Condition(reg_write_mem):
            with Condition((needs_rs1 & (rs1 == rd_mem)) | (needs_rs2 & (rs2 == rd_mem))):
                data_hazard_ex = UInt(1)(1)
        
        # 检查与WB阶段的冲突
        with Condition(mem_wb_valid & reg_write_wb):
            with Condition((needs_rs1 & (rs1 == rd_wb)) | (needs_rs2 & (rs2 == rd_wb))):
...
class ExecuteStage(Module):
    ...
    def build(self, pipeline_regs, reg_file, memory_stage):
        ...
        # MEM阶段应该继续执行，使用ex_mem_valid控制
        mem_valid_in = pipeline_regs[0].ex_mem_valid
...
class MemoryStage(Module):
    ...
    def build(self, pipeline_regs, ex_mem_result, writeback_stage):
        ...
        # WB阶段 - 使用valid_in控制执行
        # WB阶段应该继续执行，使用mem_wb_valid控制
        wb_valid_in = pipeline_regs[0].mem_wb_valid
...
class HazardUnit(Module):
    ...
    def build(self, pipeline_regs):
        ...
        # 最终输出信号
        # 直接修改流水线寄存器的valid位而不是使用stall信号
        # 当检测到数据冒险时，将IF/ID和ID/EX的valid设为0
        with Condition(data_hazard):
            # 清除IF/ID和ID/EX阶段的valid信号，阻止新指令进入流水线
        ...
        log("HAZARD: RS1={}, RS2={}, MEM_RD={}, WB_RD={}, Data_Hazard={}",
...
class Driver(Module):
    ...
    def build(self, pipeline_regs, instruction_memory, fetch_stage):
        ...
        # 调用fetch_stage，不再需要stall信号，因为HazardUnit直接控制pipeline_reg的valid位
        fetch_stage.async_called(pc_in=current_pc)
