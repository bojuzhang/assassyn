...
CONTROL_LEN = 51 # 控制信号长度 (42 + 3位mul_op + 3位div_op + 2位load_type + 1位load_unsigned)
BTB_SIZE = 64  # BTB表大小
...
class FetchStage(Module):
    ...
    def build(self, pc, stall, if_id_pc, if_id_instruction, if_id_valid, if_id_prediction_info, instruction_memory, btb, bht, btb_valid, decode_stage):
        ...
        with Condition(if_id_valid[0]):
            ...
            if_id_valid[0] = stall[0].select(if_id_valid[0], UInt(1)(1))
        ...
        # 记录取指令日志
        with Condition(if_id_valid[0] & ~stall[0]):
...
class DecodeStage(Module):
    ...
    def build(self, if_id_valid, if_id_pc, if_id_instruction, if_id_prediction_info, id_ex_pc, id_ex_control, id_ex_valid, id_ex_rs1_idx, id_ex_rs2_idx, id_ex_immediate, id_ex_need_rs1, id_ex_need_rs2, id_ex_prediction_info, reg_file, execute_stage):
        ...
        # For LBU/LHU, set the load_type to LB/LH respectively (reuse same encoding)
        ...
        store_type_bits = UInt(2)(0b10)  # 默认为SW
        ...
        control_signals = concat(
            load_unsigned,    # [50:50] 无符号加载标志: 1=LBU/LHU, 0=LB/LH/LW
            load_type_bits,   # [49:48] 加载类型: 00=LB/LBU, 01=LH/LHU, 10=LW
            div_op,           # [47:45] 除法操作码
            mul_op,           # [44:42] 乘法操作码
        ...
        # 记录指令解码日志
        with Condition(if_id_valid[0] & id_ex_valid[0]):
            # 创建指令类型字符串变量
            inst_type = is_r_type.select(UInt(8)(ord('R')), is_i_type.select(UInt(8)(ord('I')), is_l_type.select(UInt(8)(ord('L')), is_s_type.select(UInt(8)(ord('S')), is_b_type.select(UInt(8)(ord('B')), is_j_type.select(UInt(8)(ord('J')), is_jr_type.select(UInt(8)(ord('J')), UInt(8)(ord('U')))))))))
            log("ID: PC={:08x}, Instruction={:08x}, Type={}, rs1={}, rs2={}, rd={}, imm={:08x}",
                if_id_pc_in, instruction, inst_type, rs1, rs2, rd, immediate)
            # 记录特定指令
            with Condition(is_jr_type & (rd == UInt(5)(0)) & (rs1 == UInt(5)(1))):  # ret 指令: jalr x0, x1, 0
                log("ID: Detected RET instruction at PC={:08x}", if_id_pc_in)
            with Condition(is_i_type & (rd == UInt(5)(10)) & (rs1 == UInt(5)(5)) & (func3 == UInt(3)(0b000))):  # mv a0, a5
                log("ID: Detected MV instruction at PC={:08x}", if_id_pc_in)
...
class ExecuteStage(Module):
    ...
    def build(self, id_ex_valid, id_ex_pc, id_ex_rs1_idx, id_ex_rs2_idx, id_ex_immediate, id_ex_control, id_ex_prediction_info, ex_mem_pc, ex_mem_control, ex_mem_valid, ex_mem_result, ex_mem_data, reg_file, memory_stage, mem_wb_control, mem_wb_valid, mem_wb_mem_data, mem_wb_ex_result, data_sram, mul_a, mul_b, mul_op_reg, mul_start, mul_cycle_counter, mul_stage1_sum, mul_stage1_carry, mul_stage2_sum, mul_stage2_carry, mul_valid, mul_result_reg, mul_in_progress, mul_rd_reg, mul_control_reg, mul_pc_reg, div_dividend, div_divisor, div_op_reg, div_state, div_remainder, div_quotient, div_iter_count, div_sign, div_neg_result, div_valid, div_result_reg, div_rd_reg, div_control_reg, div_pc_reg):
        ...
        wb_mem_data = mem_wb_mem_data[0]   # 从 MEM/WB 流水线寄存器读取
        ...
        # 记录ALU操作
        with Condition(id_ex_valid[0] & ~is_branch & ~is_jump & ~is_jumpr):
            log("EX ALU: PC={:08x}, OP={:05b}, A={:08x}, B={:08x}, Result={:08x}",
                pc_in, alu_op, alu_a, alu_b, alu_result)
...
class MemoryStage(Module):
    ...
    @module.combinational
    def build(self, ex_mem_valid, ex_mem_result, ex_mem_pc, ex_mem_data, ex_mem_control, mem_wb_control, mem_wb_valid, mem_wb_mem_data, mem_wb_ex_result, mem_sb_sh_active, mem_sb_sh_store_type, mem_sb_sh_data, mem_sb_sh_addr, mem_sb_sh_byte_offset, writeback_stage, data_sram):
        pc_in = ex_mem_pc[0]
        ...
        byte_offset = addr_in[0:1]  # bits [1:0]

        # 获取当前状态
        sb_sh_active = mem_sb_sh_active[0]

        # ==================== SRAM控制信号计算 ====================
        # 状态0时的条件
        in_state0 = sb_sh_active == UInt(1)(0)
        valid_mem_access = mem_wb_valid[0]
        
        # Load条件：状态0 & valid & mem_read & ~mem_write
        is_load = in_state0 & valid_mem_access & mem_read & ~mem_write
        
        # SW条件：状态0 & valid & mem_write & ex_mem_valid & store_type==SW
        is_sw = in_state0 & valid_mem_access & mem_write & ex_mem_valid[0] & (store_type == UInt(2)(0b10))
        
        # SB/SH条件：状态0 & valid & mem_write & ex_mem_valid & (store_type==SB | store_type==SH)
        is_sb_sh = in_state0 & valid_mem_access & mem_write & ex_mem_valid[0] & ((store_type == UInt(2)(0b00)) | (store_type == UInt(2)(0b01)))
        
        # 状态1时需要写入
        in_state1 = sb_sh_active == UInt(1)(1)
        
        # ==================== 计算状态1需要的值（在SRAM调用前）====================
        saved_store_type = mem_sb_sh_store_type[0]
        saved_data = mem_sb_sh_data[0]
        saved_addr = mem_sb_sh_addr[0]
        saved_byte_offset = mem_sb_sh_byte_offset[0]
        
        # 上一周期读取的数据
        current_word = data_sram.dout[0]
        
        # SB指令的字节修改
        store_byte = saved_data[0:7]  # bits [7:0]
        byte_mask = UInt(32)(0xFF) << (saved_byte_offset * UInt(32)(8))
        sb_modified_word = ((current_word & ~byte_mask) | (store_byte.bitcast(UInt(32)) << (saved_byte_offset * UInt(32)(8)))).bitcast(UInt(32))

        # SH指令的半字修改
        store_half = saved_data[0:15]  # bits [15:0]
        half_mask = UInt(32)(0xFFFF) << (saved_byte_offset[1:1] * UInt(32)(16))
        sh_modified_word = ((current_word & ~half_mask) | (store_half.bitcast(UInt(32)) << (saved_byte_offset[1:1] * UInt(32)(16)))).bitcast(UInt(32))

        # 根据存储类型选择最终的写入数据
        modified_word = (saved_store_type == UInt(2)(0b00)).select(
            sb_modified_word,
            (saved_store_type == UInt(2)(0b01)).select(sh_modified_word, current_word)
        )
        
        # ==================== SRAM操作 ====================
        # 计算we/re信号（使用条件选择）
        # we = 1 当 SW 或 状态1
        # re = 1 当 Load 或 SB/SH（状态0读取）
        sram_we = (is_sw | in_state1).select(UInt(1)(1), UInt(1)(0))
        sram_re = (is_load | is_sb_sh).select(UInt(1)(1), UInt(1)(0))
        
        # 计算SRAM地址和数据
        # 状态1时使用saved_addr和modified_word，否则使用word_addr和data_in
        sram_addr = in_state1.select(saved_addr.bitcast(UInt(32)), word_addr.bitcast(UInt(32)))
        sram_wdata = in_state1.select(modified_word, data_in)
        
        # 单一SRAM调用，不在Condition块内
        data_sram.build(we=sram_we, re=sram_re, addr=sram_addr, wdata=sram_wdata)
        
        # ==================== 状态转换 ====================
        # SB/SH进入时保存状态
        with Condition(is_sb_sh):
            mem_sb_sh_store_type[0] = store_type
            mem_sb_sh_data[0] = data_in
            mem_sb_sh_addr[0] = word_addr
            mem_sb_sh_byte_offset[0] = byte_offset
            mem_sb_sh_active[0] = UInt(1)(1)
        
        # 状态1完成时回到状态0
        with Condition(in_state1):
            mem_sb_sh_active[0] = UInt(1)(0)

        # ==================== 更新MEM/WB寄存器 ====================
        # 保存SRAM读取的数据到MEM/WB流水线寄存器
        # 对于load指令，数据来自SRAM；对于其他指令，设为0
        mem_read_data = data_sram.dout[0]
        
        with Condition(in_state0 & valid_mem_access):
            ...
            mem_wb_mem_data[0] = ex_mem_valid[0].select(mem_read_data, UInt(XLEN)(0))

        writeback_stage.async_called()

        # 检测SB/SH指令是否正在进入（用于生成暂停信号）
        # 条件：当前是状态0，且有SB/SH存储指令进入
        is_sb_or_sh = mem_write & ((store_type == UInt(2)(0b00)) | (store_type == UInt(2)(0b01))) & ex_mem_valid[0]
        sb_sh_entering = (sb_sh_active == UInt(1)(0)) & is_sb_or_sh
        
        # sb_sh_stall信号：当SB/SH刚进入（状态0）或正在处理（状态1）时都需要暂停
        # 使用 select 来生成正确的 UInt(1) 类型
        sb_sh_stall_signal = (sb_sh_entering | (sb_sh_active == UInt(1)(1))).select(UInt(1)(1), UInt(1)(0))

        # 构建扩展的memory_signals，包含control信息和SB/SH暂停信号
        MEMORY_SIGNALS_LEN = CONTROL_LEN + 1  # control + sb_sh_stall
        memory_signals_extended = concat(
            sb_sh_stall_signal.bitcast(Bits(1)),  # [CONTROL_LEN] SB/SH暂停信号
            control_in.bitcast(Bits(CONTROL_LEN))  # [CONTROL_LEN-1:0] 控制信号
        )
        memory_signals = memory_signals_extended
...
class WriteBackStage(Module):
    ...
    def build(self, mem_wb_valid, mem_wb_mem_data, mem_wb_ex_result, mem_wb_control, reg_file, data_sram):
        mem_data_in = mem_wb_mem_data[0]  # Data from MEM/WB pipeline register
        ex_result_in = mem_wb_ex_result[0]  # ALU result (address for load instructions)
        control_in = mem_wb_control[0]

        # 解析控制信号
        reg_write = control_in[7:7]
        mem_to_reg = control_in[8:8]
        wb_rd = control_in[25:29]  # rd地址 (unchanged position)
        load_type = control_in[48:49]   # 加载类型: 00=LB/LBU, 01=LH/LHU, 10=LW
        load_unsigned = control_in[50:50]  # 无符号加载标志: 1=LBU/LHU, 0=LB/LH/LW

        # For load instructions, process the byte/half-word data
        # Calculate byte offset from the address (ex_result_in)
        byte_offset = ex_result_in[0:1]  # bits [1:0]

        # Extract byte based on byte_offset - convert to UInt
        extracted_byte_0 = mem_data_in[0:7].bitcast(UInt(8))   # bits [7:0]
        extracted_byte_1 = mem_data_in[8:15].bitcast(UInt(8))  # bits [15:8]
        extracted_byte_2 = mem_data_in[16:23].bitcast(UInt(8)) # bits [23:16]
        extracted_byte_3 = mem_data_in[24:31].bitcast(UInt(8)) # bits [31:24]

        # Select byte using bit[1] first, then bit[0]
        # If bit[1]=0: use byte_0 or byte_1 (depending on bit[0])
        # If bit[1]=1: use byte_2 or byte_3 (depending on bit[0])
        byte_low_high = byte_offset[1:1].select(
            byte_offset[0:0].select(extracted_byte_1, extracted_byte_0),  # bit[1]=0: if bit[0]=1 use byte_1, else byte_0
            byte_offset[0:0].select(extracted_byte_3, extracted_byte_2)   # bit[1]=1: if bit[0]=1 use byte_3, else byte_2
        )
        extracted_byte = byte_low_high

        # Extract half-word based on byte_offset[1]
        extracted_half_0 = mem_data_in[0:15].bitcast(UInt(16))  # bits [15:0]
        extracted_half_2 = mem_data_in[16:31].bitcast(UInt(16)) # bits [31:16]

        # Select half-word using bit[1]
        extracted_half = byte_offset[1:1].select(extracted_half_0, extracted_half_2)

        # Sign-extend byte (for LB)
        byte_sign_bit = extracted_byte[7:7]
        zero_extended_byte = extracted_byte.bitcast(UInt(32))
        # Use bitwise OR for sign extension, then convert back to UInt
        sign_extended_bits = extracted_byte.bitcast(Bits(32)) | Bits(32)(0xFFFFFF00)
        sign_extended_byte = sign_extended_bits.bitcast(UInt(32))
        lb_data = byte_sign_bit.select(sign_extended_byte, zero_extended_byte)

        # Zero-extend byte (for LBU)
        lbu_data = extracted_byte.bitcast(UInt(32))

        # Sign-extend half-word (for LH)
        half_sign_bit = extracted_half[15:15]
        zero_extended_half = extracted_half.bitcast(UInt(32))
        # Use bitwise OR for sign extension, then convert back to UInt
        sign_extended_half_bits = extracted_half.bitcast(Bits(32)) | Bits(32)(0xFFFF0000)
        sign_extended_half = sign_extended_half_bits.bitcast(UInt(32))
        lh_data = half_sign_bit.select(sign_extended_half, zero_extended_half)

        # Zero-extend half-word (for LHU)
        lhu_data = extracted_half.bitcast(UInt(32))

        # For LW, just use the full word
        lw_data = mem_data_in

        # Select between signed/unsigned for byte loads
        lb_lbu_data = load_unsigned.select(lbu_data, lb_data)   # If unsigned=1, use lbu_data

        # Select between signed/unsigned for half-word loads
        lh_lhu_data = load_unsigned.select(lhu_data, lh_data)   # If unsigned=1, use lhu_data

        # Select final load data based on load_type (00=LB/LBU, 01=LH/LHU, 10=LW)
        # Use nested selects: first check if it's LW (10), else check if it's LH/LHU (01)
        final_load_data = load_type[1:1].select(
            load_type[0:0].select(lh_lhu_data, lb_lbu_data),  # bit[1]=0: if bit[0]=1 use LH/LHU, else LB/LBU
            lw_data  # bit[1]=1: it's LW
        )

        # 选择写回数据
        # For load instructions (mem_to_reg=1), use the processed byte/half-word/word data
        # For other instructions (mem_to_reg=0), use the ALU result
        wb_data = mem_to_reg.select(final_load_data, ex_result_in)
        ...
        with Condition(mem_wb_valid[0] & reg_write):
            ...
            # 特别关注a0寄存器(x10)的写回
            with Condition(wb_rd == UInt(5)(10)):
                log("WB: Wrote to a0(x10) = {}", wb_data)
...
class HazardUnit(Downstream):
    ...
    def build(self, pc, stall, if_id_valid, if_id_instruction, if_id_prediction_info, id_ex_control, id_ex_valid, id_ex_rs1_idx, id_ex_rs2_idx, id_ex_immediate, id_ex_prediction_info, ex_mem_valid, mem_wb_valid, btb, bht, btb_valid, fetch_signals, decode_signals, execute_signals, memory_signals, writeback_signals, mul_in_progress, mul_cycle_counter, div_state, div_iter_count):
        ...
        # 解析memory_signals，提取SB/SH暂停信号
        mem_control = memory_signals[0:CONTROL_LEN-1].bitcast(Bits(CONTROL_LEN))
        ...
        mem_stage_control = memory_signals[0:CONTROL_LEN-1].bitcast(UInt(CONTROL_LEN))
        mem_stage_control = ex_mem_valid[0].select(mem_stage_control, UInt(CONTROL_LEN)(0))
        rd_mem = mem_stage_control[25:29]
        reg_write_mem = mem_stage_control[7:7]
        mem_read_mem = mem_stage_control[5:5]  # 解析 mem_read 信号用于检测 Load-Use 冒险
        ...
        # 获取EX阶段控制信号（用于Load-Use hazard检测和乘除法检测）
        ex_control = id_ex_control[0]
        
        # EX 阶段 Load-Use 冒险检测 (关键！这是正确的检测时机)
        # 当Load指令在EX阶段时，其结果还没从内存读取，无法forward
        ex_mem_read_hazard = ex_control[5:5]
        ex_reg_write_hazard = ex_control[7:7]
        ex_rd_hazard = ex_control[25:29]
        load_use_hazard_ex = (ex_mem_read_hazard & ex_reg_write_hazard & (ex_rd_hazard != UInt(5)(0)) &
                              ((needs_rs1 & (rs1 == ex_rd_hazard)) | (needs_rs2 & (rs2 == ex_rd_hazard))))

        # MEM 阶段 Load-Use 冒险（备用，理论上MEM阶段可以forward）
        ...
        # ==================== 乘法冒险检测 ====================
        # 检测EX阶段是否有乘法指令（ex_control已在上面定义）
        ...
        # 6. SB/SH指令执行中（需要等待读-修改-写完成）
        # sb_sh_stall 已从 memory_signals 中提取，包含了 SB/SH 指令进入和处理期间的暂停信号
        data_hazard = ((load_use_hazard_ex | load_use_hazard_mem | mul_executing | mul_result_hazard | div_executing | div_result_hazard | sb_sh_stall) & ~need_flush)
        # log("HAZARD_DEBUG: load_use_ex={}, load_use_mem={}, if_id_valid={}, ex_rs1={}, ex_rs2={}, needs_rs1={}, needs_rs2={}, rs1={}, rs2={}, ex_rd={}, data_hazard={}",
        #     load_use_hazard_ex, load_use_hazard_mem, if_id_valid[0], ex_control[20:24], ex_control[15:19], needs_rs1, needs_rs2, rs1, rs2, ex_rd_hazard, data_hazard)
        # log("HAZARD2: data_hazard={}, need_flush={}, mul_executing={}, mul_result_hazard={}, div_executing={}, div_result_hazard={}, sb_sh_stall={}",
        #     data_hazard, need_flush, mul_executing, mul_result_hazard, div_executing, div_result_hazard, sb_sh_stall)
        ...
        # Load-Use hazard时：EX阶段指令继续执行（Load需要完成），只暂停IF/ID阶段
        # 所以 id_ex_valid 不应该被 data_hazard 清除
        # 但对于 need_flush，EX阶段指令确实需要清空
        id_ex_valid[0] = (~need_flush)
        # if_id_valid[0] = (~data_hazard)  # 由FetchStage处理
        ...
        # ID/EX阶段刷新: 清空所有寄存器
        with Condition(if_id_valid[0] & ~data_hazard):  # 只有在有效且无数据冒险时才更新IF/ID寄存器
        ...
        with Condition(id_ex_valid[0]):
            id_ex_control[0] = (need_flush | data_hazard).select(nop_control, control_in)
            id_ex_immediate[0] = (need_flush | data_hazard).select(UInt(XLEN)(0), immediate)
            id_ex_rs1_idx[0] = (need_flush | data_hazard).select(UInt(5)(0), rs1)
            id_ex_rs2_idx[0] = (need_flush | data_hazard).select(UInt(5)(0), rs2)
            id_ex_prediction_info[0] = (need_flush | data_hazard).select(UInt(PREDICTION_INFO_LEN)(0), prediction_info_id)
...
def build_cpu(program_file="test_program.txt"):
    ...
    with sys:
        ...
        # SB/SH指令状态寄存器（1位即可）
        mem_sb_sh_active = RegArray(UInt(1), 1, initializer=[0])       # 0=正常, 1=SB/SH处理

        # SB/SH指令需要保存的信息（用于状态1时使用）
        mem_sb_sh_store_type = RegArray(UInt(2), 1, initializer=[0])   # 00=SB, 01=SH
        mem_sb_sh_data = RegArray(UInt(XLEN), 1, initializer=[0])      # 要存储的数据
        mem_sb_sh_addr = RegArray(UInt(XLEN), 1, initializer=[0])      # 字地址
        mem_sb_sh_byte_offset = RegArray(UInt(2), 1, initializer=[0])  # 字节偏移 (bits [1:0])
        ...
        memory_signals = memory_stage.build(ex_mem_valid, ex_mem_result, ex_mem_pc, ex_mem_data, ex_mem_control, mem_wb_control, mem_wb_valid, mem_wb_mem_data, mem_wb_ex_result, mem_sb_sh_active, mem_sb_sh_store_type, mem_sb_sh_data, mem_sb_sh_addr, mem_sb_sh_byte_offset, writeback_stage, data_sram)
